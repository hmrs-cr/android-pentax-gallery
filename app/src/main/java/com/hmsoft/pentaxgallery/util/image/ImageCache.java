...
public class ImageCache {
    ...
    private static final int DEFAULT_MEM_CACHE_SIZE = 1024 * 32; // 32MB

    // Default disk cache size in bytes
    private static final int DEFAULT_DISK_CACHE_SIZE = 1024 * 1024 * 128; // 128MB
                ...
                if (mCacheParams.diskCacheEnabled && diskCacheDir != null) {
                    ...
                    if (CacheUtils.getUsableSpace(diskCacheDir) > mCacheParams.diskCacheSize) {
                        try {
                            ...
                            if (BuildConfig.DEBUG) {
                                Logger.debug(TAG, "Disk cache initialized");
                            }
                        } catch (final IOException e) {
                            mCacheParams.diskCacheDir = null;
                            Logger.error(TAG, "initDiskCache - " + e);
                        }
                    }
                }
            }
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    Logger.error(TAG, "addBitmapToCache - " + e);
                } catch (Exception e) {
                    Logger.error(TAG, "addBitmapToCache - " + e);
                } finally {
                    try {
    ...
    public BitmapDrawable getBitmapFromMemCache(String data) {
        ...
        if (BuildConfig.DEBUG && memValue != null) {
            Logger.debug(TAG, "Memory cache hit");
        }
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    if (snapshot != null) {
                        if (BuildConfig.DEBUG) {
                            Logger.debug(TAG, "Disk cache hit");
                        }
                    ...
                    Logger.error(TAG, "getBitmapFromDiskCache - " + e);
                } finally {
                    try {
                        if (inputStream != null) {
                            inputStream.close();
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
                try {
                    ...
                    if (BuildConfig.DEBUG) {
                        Logger.debug(TAG, "Disk cache cleared");
                    }
                } catch (IOException e) {
                    Logger.error(TAG, "clearCache - " + e);
                }
                mDiskLruCache = null;
                initDiskCache();
            }
        ...
        synchronized (mDiskCacheLock) {
            if (mDiskLruCache != null) {
                try {
                    mDiskLruCache.flush();
                    if (BuildConfig.DEBUG) {
                        Logger.debug(TAG, "Disk cache flushed");
                    }
                } catch (IOException e) {
            ...
            if (mDiskLruCache != null) {
                try {
                    if (!mDiskLruCache.isClosed()) {
                        ...
                        mDiskLruCache = null;
                        if (BuildConfig.DEBUG) {
                            Logger.debug(TAG, "Disk cache closed");
                        }
                    }
                } catch (IOException e) {
                    Logger.error(TAG, "close - " + e);
                }
            }
        }
    }
